...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_dct3s_textured_gaussians_kernel(
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                if (valid_texture > 0)
                {
                    ...
                    if (texture_alpha)
                    {
                        alpha_scaling_factor = sigmoid(dct3::sample(textures, texture_res, g, u, v, ucos, vcos, alpha_channel));
                    }
                }
                else
                {
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_fwd_dct3s_g_kernel_with_dim(
        // Gaussian parameters
        ...
        const float g_weight)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_dct3s_textured_gaussians_kernel<CDIM, float>,
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    rasterize_to_pixels_fwd_dct3s_textured_gaussians_tensor(
        // Gaussian parameters
        const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
        const torch::Tensor &ray_transforms,            // [C, N, 3, 3] or [nnz, 3, 3]
        const torch::Tensor &colors,                    // [C, N, channels] or [nnz, channels]
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_fwd_dct3s_g_kernel_with_dim<N>(        \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
