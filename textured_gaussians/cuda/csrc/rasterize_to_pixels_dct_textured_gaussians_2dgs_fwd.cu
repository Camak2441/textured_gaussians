...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_dct_textured_gaussians_kernel(
        ...
        S *__restrict__ gs_contrib_count)
    {
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                const S u = (S)((s.x + texture_range.x) / (texture_range.x * 2) * (texture_res_x - 2) + S(1)) / texture_res_x;
                const S v = (S)((s.y + texture_range.y) / (texture_range.y * 2) * (texture_res_y - 2) + S(1)) / texture_res_y;
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_fwd_dct_g_kernel_with_dim(
        // Gaussian parameters
        ...
        const float gs_contrib_threshold)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_dct_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_fwd_dct_g_kernel_with_dim<N>(          \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
