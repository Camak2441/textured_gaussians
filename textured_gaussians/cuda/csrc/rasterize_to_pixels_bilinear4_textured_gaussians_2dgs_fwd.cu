...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_bilinear4_textured_gaussians_kernel(
        ...
        const float g_weight)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_bilinear4_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_fwd_tg4_kernel_with_dim<N>(            \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
