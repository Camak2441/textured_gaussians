...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_aniso_bilinear_textured_sigmoids_kernel(
        ...
        S *__restrict__ gs_contrib_count)
    {
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                S alpha = opac * (S(0.998) - s_weight + s_weight * step_vis);
                if (valid_texture > 0)
                {
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_fwd_aniso_bilinear_s_kernel_with_dim(
        // Gaussian parameters
        ...
        const float s_weight)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_aniso_bilinear_textured_sigmoids_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                       \
    case N:                                                  \
        return call_fwd_aniso_bilinear_s_kernel_with_dim<N>( \
            means2d,                                         \
            steepnesses,                                     \
            ray_transforms,                                  \
            colors,                                          \
