...
namespace gsplat
{
    ...
    __global__ void rasterize_dct3_textures_kernel(
    ...
    )
    {
        ...
        int texture_res = floor(sqrt(textures.size(1) * 2));
        ...
        S u = (((S)pixel_x) / (image_width - 1) * (texture_res - 2) + S(1)) / texture_res;
        S v = (((S)pixel_y) / (image_height - 1) * (texture_res - 2) + S(1)) / texture_res;
        dct3::precompute(texture_res, u, v, ucos, vcos);

        dct3::color_sample<COLOR_DIM, S>(textures, texture_res, g, u, v, ucos, vcos, render_colors);
    }

    template <uint32_t CDIM>
    torch::Tensor
    call_dct3_kernel_with_dim(
        // Gaussian parameters
        ...
        const uint32_t tile_size)
    {
        ...
        const uint32_t shared_mem =
            tile_size * tile_size * sizeof(float) * (texture_res * 2);

        if (cudaFuncSetAttribute(
                rasterize_dct3_textures_kernel<CDIM, float>,
    ...
    torch::Tensor
    rasterize_dct3_textures_tensor(
        // Gaussian parameters
        const torch::Tensor &textures, //
        // image size
        const uint32_t image_width,
...
#define __GS__CALL_(N)                       \
    case N:                                  \
        return call_dct3_kernel_with_dim<N>( \
            textures,                        \
            image_width,                     \
            image_height,                    \
            tile_size);
