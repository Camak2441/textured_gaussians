...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_aniso_bilinear_textured_gaussians_kernel(
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_fwd_aniso_bilinear_g_kernel_with_dim(
        // Gaussian parameters
        ...
        const float g_weight)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_aniso_bilinear_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                       \
    case N:                                                  \
        return call_fwd_aniso_bilinear_g_kernel_with_dim<N>( \
            means2d,                                         \
            ray_transforms,                                  \
            colors,                                          \
            opacities,                                       \
