...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_bilinear3_textured_gaussians_kernel(
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    bilinear3::precompute(s.x, s.y, texture_res_x, texture_res_y, texture_range.x, texture_range.y, ucoords, vcoords, bilerp_weights);
                    ...
                    if (texture_alpha && valid_texture > 0)
                    {
                        ...
                        {
                            alpha_scaling_factor += bilerp_weights[i] * bilinear3::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], alpha_channel);
                        }
                    }
                    else
                    {
                ...
                if (valid)
                {
                    ...
                    for (uint32_t k = 0; k < COLOR_DIM; ++k)
                    {
                        ...
                        if (texture_color && valid_texture > 0)
                        {
                            ...
                            for (uint32_t i = 0; i < 4; ++i)
                            {
                                bilinear3::update(v_textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], k, fac * bilerp_weights[i] * v_render_c[k]);
                                tex_colors[k] += bilerp_weights[i] * bilinear3::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], k);
                            }
                        }
                    }
                    ...
                    if (opac * vis * alpha_scaling_factor <= S(0.999))
                    {
                        ...
                        if (texture_alpha && valid_texture > 0)
                        {
                            for (uint32_t i = 0; i < 4; ++i)
                            {
                                bilinear3::update(v_textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], alpha_channel, bilerp_weights[i] * vis * opac * v_alpha);
                            }
                        }
                    }
                    ...
                    {
                        // Recover fractional UV position from precomputed bilerp_weights
                        const S w_u = bilerp_weights[1] + bilerp_weights[3];
                        const S w_v = bilerp_weights[2] + bilerp_weights[3];

                        // d(bilerp_weights[i])/d(u) and d(bilerp_weights[i])/d(v)
                        ...
                        // Contribution from texture color channels changing with UV
                        if (texture_color)
                        {
                            GSPLAT_PRAGMA_UNROLL
                            for (uint32_t k = 0; k < COLOR_DIM; ++k)
                            {
                                ...
                                for (uint32_t i = 0; i < 4; ++i)
                                {
                                    const S tex_val = bilinear3::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], k);
                        ...
                        if (texture_alpha && opac * vis * alpha_scaling_factor <= S(0.999))
                        {
                            ...
                            for (uint32_t i = 0; i < 4; ++i)
                            {
                                const S tex_val_a = bilinear3::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], alpha_channel);
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_bilinear3_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_bwd_tg3_kernel_with_dim<N>(            \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
