...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd2_textured_gaussians_kernel(
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    valid_texture = bilinear::precompute(s.x, s.y, texture_res_x, texture_res_y, ucoords, vcoords, bilerp_weights);
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd2_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                         \
    case N:                                    \
        return call_bwd2_t_kernel_with_dim<N>( \
            means2d,                           \
            ray_transforms,                    \
            colors,                            \
            opacities,                         \
