...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_mip_textured_gaussians_kernel(
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    const vec3<S> minh_x = h_u - S(0.5) * w_M;
                    const vec3<S> maxh_x = h_u + S(0.5) * w_M;
                    const vec3<S> minh_y = h_v - S(0.5) * w_M;
                    const vec3<S> maxh_y = h_v + S(0.5) * w_M;
                    ...
                    valid_texture = mip::precompute(
                        s.x, s.y, dsdx, dsdy, texture_res_x, texture_res_y,
                ...
                if (valid)
                {
                    ...
                    for (uint32_t k = 0; k < COLOR_DIM; ++k)
                    {
                        ...
                        if (valid_texture > 0)
                        {
                            ...
                            tex_colors[k] += mip::efficient_sample(textures, g, k, ucoords, vcoords, tcoords, trilerp_weights);
                        }
                    }
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_bwd_mip_g_kernel_with_dim(
        // Gaussian parameters
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_mip_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_bwd_mip_g_kernel_with_dim<N>(          \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
