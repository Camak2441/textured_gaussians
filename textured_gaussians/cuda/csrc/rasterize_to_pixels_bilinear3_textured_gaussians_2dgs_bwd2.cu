...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd2_bilinear3_textured_gaussians_kernel(
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    if (valid_texture > 0)
                    {
                        ...
                        {
                            alpha_scaling_factor += bilerp_weights[i] * bilinear3::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], 3);
                        }
                    }
                    else
                    {
                ...
                if (valid)
                {
                    ...
                    if (valid_texture > 0)
                    {
                        ...
                        if (opac * vis * alpha_scaling_factor <= S(0.999))
                        {
                            ...
                            for (uint32_t i = 0; i < 4; ++i)
                            {
                                const S tex_val_a = bilinear3::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], 3);
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd2_bilinear3_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_bwd2_tg3_kernel_with_dim<N>(           \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
