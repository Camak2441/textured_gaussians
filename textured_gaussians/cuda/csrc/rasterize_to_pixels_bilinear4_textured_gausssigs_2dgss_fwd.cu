...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_bilinear4_textured_gausssigs_kernel(
        ...
        const float s_weight)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_bilinear4_textured_gausssigs_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_fwd_tgs4_kernel_with_dim<N>(           \
            means2d,                                       \
            steepnesses,                                   \
            ray_transforms,                                \
            colors,                                        \
