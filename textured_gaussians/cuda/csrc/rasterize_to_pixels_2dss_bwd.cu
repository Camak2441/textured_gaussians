...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_2dss_kernel(
                ...
                if (valid)
                {
                    ...
                    if (opac * vis <= S(0.999))
                    {
                        ...
                        if (gauss_weight_3d <= gauss_weight_2d)
                        {
                            ...
                            // @see rasterize_to_pixels_2dss_fwd.cu to understand what is going on here
                        ...
                        else
                        {
                            ...
                            const S v_G_ddelx = -vis * vis * S(2) * steepness * FILTER_INV_SQUARE * d.x * gw_pow;
                            const S v_G_ddely = -vis * vis * S(2) * steepness * FILTER_INV_SQUARE * d.y * gw_pow;
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_2dss_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                           \
    case N:                                      \
        return call_bwd_2dss_kernel_with_dim<N>( \
            means2d,                             \
            steepnesses,                         \
            ray_transforms,                      \
            colors,                              \
