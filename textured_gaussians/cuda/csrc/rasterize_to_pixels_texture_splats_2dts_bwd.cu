...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_texture_splats_kernel(
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_texture_splats_kernel<CDIM, float>,
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    rasterize_to_pixels_bwd_texture_splats_tensor(
        // Gaussian parameters
        const torch::Tensor &means2d,        // [C, N, 2] or [nnz, 2]
        const torch::Tensor &ray_transforms, // [C, N, 3, 3] or [nnz, 3, 3]
        const torch::Tensor &colors,         // [C, N, 3] or [nnz, 3]
