        ...
        S *__restrict__ gs_contrib_count)
    {
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                int32_t vcoords[4];
                S bilerp_weights[4];
                int32_t valid_texture = bilinear2::precompute(s.x, s.y, texture_res_x, texture_res_y, texture_range.x, texture_range.y, ucoords, vcoords, bilerp_weights);
                ...
                for (uint32_t k = 0; k < COLOR_DIM; ++k)
                {
                    ...
                    if (texture_color && valid_texture > 0)
                    {
                        for (uint32_t i = 0; i < 4; ++i)
                        {
                            tex_color += bilerp_weights[i] * bilinear2::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], k);
                        }
                    }
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_fwd_tg2_kernel_with_dim<N>(            \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
