...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_2dss_kernel(
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                else
                {
                    ...
                    alpha = min(S(0.999), opac * sigmoid(-(steepness * log(gauss_weight))));
                }
        ...
        const float gs_contrib_threshold)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_2dss_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                           \
    case N:                                      \
        return call_fwd_2dss_kernel_with_dim<N>( \
            means2d,                             \
            steepnesses,                         \
            ray_transforms,                      \
            colors,                              \
