...
namespace gsplat
{
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    valid_texture = mip2::precompute(
                        s.x, s.y, dsdx, dsdy, texture_res, log_texture_res,
                        texture_range.x, texture_range.y,
                        mipcoords, trilerp_weights);
                    ...
                    if (texture_alpha && valid_texture > 0)
                    {
                        ...
                        {
                            alpha_scaling_factor += trilerp_weights[i] * textures[g][mipcoords[i]][alpha_channel];
                        }
                    }
                    else
                    {
                ...
                if (valid)
                {
                    ...
                    if (opac * vis * alpha_scaling_factor <= S(0.999))
                    {
                        ...
                        if (texture_alpha && valid_texture > 0)
                        {
                            ...
                            for (uint32_t i = 0; i < 8; ++i)
                            {
                                gpuAtomicAdd(&v_textures[g][mipcoords[i]][alpha_channel], trilerp_weights[i] * vis * opac * v_alpha);
                            }
                        }
                    }
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_bwd_mip2_g_kernel_with_dim(
        // Gaussian parameters
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_bwd_mip2_g_kernel_with_dim<N>(         \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
