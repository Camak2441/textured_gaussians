...
namespace gsplat
{
    ...
    /****************************************************************************
     * Rasterization to Pixels Backward Pass Textured Gaussians (DCT3)
     ****************************************************************************/
    template <uint32_t COLOR_DIM, typename S>
    __global__ void rasterize_to_pixels_bwd_dct3_textured_gaussians_kernel(
        ...
        const S *__restrict__ ray_transforms,                                   // [C, N, 3, 3] or [nnz, 9]
        const S *__restrict__ colors,                                           // [C, N, COLOR_DIM] or [nnz, COLOR_DIM]
        const S *__restrict__ normals,                                          // [C, N, 3] or [nnz, 3]
        const S *__restrict__ opacities,                                        // [C, N] or [nnz]
        at::PackedTensorAccessor32<const S, 3, at::RestrictPtrTraits> textures, // [N, t*(t+1)/2, channels]
        ...
        S *__restrict__ v_densify)
    {
        ...
        const uint32_t alpha_channel = textures.size(2) - 1;
         ...
         * ucos/vcos/ducos/dvcos stored column-major: [coefficient][thread]
         * Thread tr's coefficient i lives at ucos[i * block_size + tr].
         * All warp threads reading coefficient i simultaneously hit consecutive banks.
         */
        ...
        S *ucos = (S *)(&normals_batch[block_size * 3]);    // [texture_res * block_size]
        S *vcos = (S *)(&ucos[block_size * texture_res]);   // [texture_res * block_size]
        S *ducos = (S *)(&vcos[block_size * texture_res]);  // [texture_res * block_size]
        S *dvcos = (S *)(&ducos[block_size * texture_res]); // [texture_res * block_size]
        ...
        // Column-major per-thread offset
        ucos += tr;
        vcos += tr;
        ducos += tr;
        dvcos += tr;
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    if (valid_texture > 0)
                    {
                        ...
                        if (texture_alpha)
                        {
                            alpha_scaling_factor = min(max(S(0), dct3::sample(textures, texture_res, g, u, v, ucos, vcos, alpha_channel, block_size)), S(1));
                        }
                        else
                        {
                ...
                // Hoisted variables shared across Part A, passes, and Part B
                 ...
                 * Triangular loop: j in [0, texture_res), i in [0, texture_res-j)
                 * flat index counts the triangular coefficient position.
                 */
                if (texture_color)
                {
                ...
                if (texture_alpha)
                {
                    ...
                    for (uint32_t tj = 0; tj < texture_res; ++tj)
                    {
                        ...
                        for (uint32_t ti = 0; ti < texture_res - tj; ++ti)
                        {
                            ...
                            S per_pix = do_alpha ? v_asf * uivj : S(0);
                            if (alpha_scaling_factor <= 0)
                            {
                                per_pix = -abs(v_asf) * uivj;
                            }
                            else if (alpha_scaling_factor >= 1)
                            {
                                per_pix = abs(v_asf) * uivj;
                            }
                            const S wsum = cg::reduce(warp, per_pix, cg::plus<S>());
    ...
    std::tuple<
        ...
        torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
    call_bwd_dct3_g_kernel_with_dim(
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_dct3_textured_gaussians_kernel<CDIM, float>,
    ...
    std::tuple<
        torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
        torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
    rasterize_to_pixels_bwd_dct3_textured_gaussians_tensor(
        const torch::Tensor &means2d,
        const torch::Tensor &ray_transforms,
        const torch::Tensor &colors,
        const torch::Tensor &opacities,
...
#define __GS__CALL_(N)                                       \
    case N:                                                  \
        return call_bwd_dct3_g_kernel_with_dim<N>(           \
