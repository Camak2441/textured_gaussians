...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_dct3_textured_gaussians_kernel(
        ...
        at::PackedTensorAccessor32<const S, 3, at::RestrictPtrTraits> textures, // [C, N, TEXTURE_DIM] or [nnz, TEXTURE_DIM] // Gaussian textures or ND features.
        ...
        at::PackedTensorAccessor32<S, 3, at::RestrictPtrTraits> v_textures, // [C, N, TEXTURE_DIM] or [nnz, TEXTURE_DIM]
        ...
        S *__restrict__ v_densify)
    {
        ...
        ucos += tr * texture_res;
        vcos += tr * texture_res;
        ducos += tr * texture_res;
        dvcos += tr * texture_res;
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    if (valid_texture > 0)
                    {
                        dct3::grad_precompute(texture_res, u, v, ucos, vcos, ducos, dvcos);
                        if (texture_alpha)
                        {
                ...
                if (valid)
                {
                    ...
                    {
                        dct3::color_sample_and_update<COLOR_DIM, S>(textures, v_textures, texture_res, g, u, v, ucos, vcos, tex_colors, deltas);
                    }
                    ...
                    if (opac * vis * alpha_scaling_factor <= S(0.999))
                    {
                        ...
                        {
                            dct3::update(v_textures, texture_res, g, u, v, ucos, vcos, alpha_channel, vis * opac * v_alpha);
                        }
                    }
                    ...
                    if (texture_gradients && valid_texture > 0)
                    {
                        ...
                        if (texture_color)
                        {
                            dct3::color_sample_grad<COLOR_DIM, S>(
