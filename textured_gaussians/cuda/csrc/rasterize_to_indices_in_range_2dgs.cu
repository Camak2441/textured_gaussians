...
namespace gsplat
{
    ...
    )
    {
        ...
        vec3<T> *xy_opacity_batch =
            reinterpret_cast<vec3<T> *>(&id_batch[block_size]); // [block_size]
        vec3<T> *u_Ms_batch =
            reinterpret_cast<vec3<T> *>(&xy_opacity_batch[block_size]); // [block_size]
        vec3<T> *v_Ms_batch =
            reinterpret_cast<vec3<T> *>(&u_Ms_batch[block_size]); // [block_size]
        vec3<T> *w_Ms_batch =
            reinterpret_cast<vec3<T> *>(&v_Ms_batch[block_size]); // [block_size]
        ...
        for (uint32_t b = range_start; b < min(range_end, num_batches); ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                const T sigma = T(0.5) * gauss_weight;
                T alpha = min(T(0.999), opac * exp(-sigma));

                if (sigma < T(0) || alpha < T(1) / T(255))
                {
                    continue;
                }

                next_trans = trans * (T(1) - alpha);
                if (next_trans <= 1e-4)
