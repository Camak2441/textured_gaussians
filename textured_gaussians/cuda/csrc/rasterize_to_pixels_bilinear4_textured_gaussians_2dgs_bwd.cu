...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_bilinear4_textured_gaussians_kernel(
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    if (texture_alpha && valid_texture > 0)
                    {
                        ...
                        {
                            alpha_scaling_factor += bilerp_weights[i] * textures[g][vcoords[i]][ucoords[i]][alpha_channel];
                        }
                    }
                    else
                    {
                ...
                if (valid)
                {
                    ...
                    if (opac * vis * alpha_scaling_factor <= S(0.999))
                    {
                        ...
                        if (texture_alpha && valid_texture > 0)
                        {
                            for (uint32_t i = 0; i < 4; ++i)
                            {
                                gpuAtomicAdd(&v_textures[g][vcoords[i]][ucoords[i]][alpha_channel], bilerp_weights[i] * vis * opac * v_alpha);
                            }
                        }
                    }
                    ...
                    if (texture_gradients && valid_texture > 0)
                    {
                        ...
                        // d(u)/d(s.x) and d(v)/d(s.y) from the bilinear4::precompute mapping
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_bilinear4_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_bwd_tg4_kernel_with_dim<N>(            \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
