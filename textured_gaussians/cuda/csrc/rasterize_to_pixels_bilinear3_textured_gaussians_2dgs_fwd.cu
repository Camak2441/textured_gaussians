...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_bilinear3_textured_gaussians_kernel(
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                for (uint32_t k = 0; k < COLOR_DIM; ++k)
                {
                    ...
                    if (texture_color && valid_texture > 0)
                    {
                        for (uint32_t i = 0; i < 4; ++i)
                        {
                            tex_color += bilerp_weights[i] * bilinear3::sample(textures, texture_res_x, texture_res_y, g, vcoords[i], ucoords[i], k);
                        }
                    }
                ...
                {
                    gpuAtomicAdd(&gs_contrib_sum[g], alpha);
                    gpuAtomicAdd(&gs_contrib_count[g], S(1));
                }
            }
        }
        if (inside)
        ...
        const float g_weight)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_bilinear3_textured_gaussians_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_fwd_tg3_kernel_with_dim<N>(            \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
