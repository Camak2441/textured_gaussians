...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_aniso_bilinear2_textured_gausssigs_kernel(
        ...
        S *__restrict__ gs_contrib_count)
    {
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                vec2<S> s0 = anisotropic_bilinear2::s_to_uv(
                ...
                if (valid_texture > 0)
                {
                    anisotropic_bilinear2::alpha_color_sample<COLOR_DIM, 3, S>(
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_fwd_aniso_bilinear2_gs_kernel_with_dim(
        // Gaussian parameters
        ...
        const float s_weight)
    {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_aniso_bilinear2_textured_gausssigs_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                         \
    case N:                                                    \
        return call_fwd_aniso_bilinear2_gs_kernel_with_dim<N>( \
            means2d,                                           \
            steepnesses,                                       \
            ray_transforms,                                    \
            colors,                                            \
