...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_fwd_dct3_textured_gaussians_kernel(
        ...
        at::PackedTensorAccessor32<const S, 3, at::RestrictPtrTraits> textures, // [N, Texture_Resolution, Texture_Resolution, 4]
        ...
        S *__restrict__ gs_contrib_count)
    {
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t)
            {
                ...
                const S u = (S)((s.x + texture_range.x) / (texture_range.x * 2) * (texture_res - 2) + S(1)) / texture_res;
                const S v = (S)((s.y + texture_range.y) / (texture_range.y * 2) * (texture_res - 2) + S(1)) / texture_res;

                // calculate alpha texture scaling factor
                S alpha_scaling_factor = S(0);
                if (valid_texture > 0)
                {
                    dct3::precompute(texture_res, u, v, ucos, vcos);
                    if (texture_alpha)
                    {
                        alpha_scaling_factor = min(max(S(0), dct3::sample(textures, texture_res, g, u, v, ucos, vcos, alpha_channel)), S(1));
                    }
                }
                else
                {
                ...
                {
                    dct3::color_sample<COLOR_DIM, S>(textures, texture_res, g, u, v, ucos, vcos, tex_color);
                }
                ...
                // if alpha is larger than the threshold, we consider this a "valid gaussian" for rendering
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_fwd_dct3_g_kernel_with_dim(
        // Gaussian parameters
        ...
        const float g_weight)
    {
        ...
        const uint32_t shared_mem =
            ...
            (sizeof(int32_t) + sizeof(vec3<float>) + sizeof(vec3<float>) +
             sizeof(vec3<float>) + sizeof(vec3<float>) + sizeof(float) * (texture_res * 2));

        // TODO: an optimization can be done by passing the actual number of
        // channels into the kernel functions and avoid necessary global memory
        // writes. This requires moving the channel padding from python to C side.
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_fwd_dct3_textured_gaussians_kernel<CDIM, float>,
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    rasterize_to_pixels_fwd_dct3_textured_gaussians_tensor(
        // Gaussian parameters
        const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
        const torch::Tensor &ray_transforms,            // [C, N, 3, 3] or [nnz, 3, 3]
        const torch::Tensor &colors,                    // [C, N, channels] or [nnz, channels]
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_fwd_dct3_g_kernel_with_dim<N>(         \
            means2d,                                       \
            ray_transforms,                                \
            colors,                                        \
            opacities,                                     \
