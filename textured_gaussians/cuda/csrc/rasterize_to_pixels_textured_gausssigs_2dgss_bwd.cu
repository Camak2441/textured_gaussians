...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_textured_gausssigs_kernel(
        const uint32_t C,
        const uint32_t N,
        ...
        S *__restrict__ v_densify)
    {
        ...
        // | id | xy_opacity | steepness | u_M | v_M | w_M | rgb | normal |
                ...
                if (valid)
                {
                    ...
                    for (uint32_t k = 0; k < COLOR_DIM; ++k)
                    {
                        ...
                        if (texture_color && valid_texture > 0)
                        {
                            ...
                            {
                                gpuAtomicAdd(
                                    &v_textures[g][vcoords[i]][ucoords[i]][k],
                                    fac * bilerp_weights[i] * v_render_c[k]);
                                tex_colors[k] +=
                                    bilerp_weights[i] * textures[g][vcoords[i]][ucoords[i]][k];
                            }
                        }
                    }
                    ...
                    if (opac * vis * alpha_scaling_factor <= S(0.999))
                    {
                        ...
                        // gw_pow = gw^(k-1) for the sigmoid term: (1-sigmoid_kernel)/(sigmoid_kernel*gw)
                        ...
                        // only sigmoid term depends on steepness: d(vis)/d(k) = s_weight * d(sigmoid_kernel)/d(k)
                        if (gauss_weight > S(0))
                        {
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_textured_gausssigs_kernel<CDIM, float>,
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    rasterize_to_pixels_bwd_textured_gausssigs_tensor(
        const torch::Tensor &means2d,
        const torch::Tensor &steepnesses,
        const torch::Tensor &ray_transforms,
        const torch::Tensor &colors,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_bwd_tgs_kernel_with_dim<N>(            \
            means2d,                                       \
            steepnesses,                                   \
            ray_transforms,                                \
            colors,                                        \
