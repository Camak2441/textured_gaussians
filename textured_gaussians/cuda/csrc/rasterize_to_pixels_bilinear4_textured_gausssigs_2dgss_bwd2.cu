...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd2_bilinear4_textured_gausssigs_kernel(
        const uint32_t C,
        const uint32_t N,
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_bwd2_tgs4_kernel_with_dim(
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd2_bilinear4_textured_gausssigs_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                     \
    case N:                                                \
        return call_bwd2_tgs4_kernel_with_dim<N>(          \
            means2d,                                       \
            steepnesses,                                   \
            ray_transforms,                                \
            colors,                                        \
