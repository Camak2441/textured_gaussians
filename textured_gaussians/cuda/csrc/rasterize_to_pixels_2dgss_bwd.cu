...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_2dgss_kernel(
        ...
        const S *__restrict__ steepnesses,
        ...
        S *__restrict__ v_steepnesses,       // [C, N] or [nnz]
        ...
        S *__restrict__ v_densify)
    {
         ...
         * | pix_x : pix_y : opac | steepness | u_M : v_M : w_M | rgb : normal |
         * ==============================
         */
        ...
        vec3<S> *u_Ms_batch = reinterpret_cast<vec3<S> *>(&steepnesses_batch[block_size]); // [block_size]
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            if (idx >= range_start)
            {
                ...
                const S steepness = steepnesses[g];
                xy_opacity_batch[tr] = {xy.x, xy.y, opac};
                steepnesses_batch[tr] = steepness;
                u_Ms_batch[tr] = {
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    else
                    {
                        ...
                        sigmoid_kernel = sigmoid(-(steepness * log(gauss_weight)));
                        vis = gaussian_kernel * (1 - s_weight) + sigmoid_kernel * s_weight;
                    }
                    alpha = min(S(0.999), opac * vis); // clipped alpha
                ...
                if (valid)
                {
                    ...
                    if (opac * vis <= S(0.999))
                    {
                        ...
                        if (gauss_weight_3d <= gauss_weight_2d)
                        {
                            ...
                            const vec2<S> v_s = v_gs * (1 - s_weight) + v_ss * s_weight;

                            // backward through the projective transform
                            // @see rasterize_to_pixels_2dgss_fwd.cu to understand what is going on here
                        ...
                        {
                            // computing the derivative of G_i w.r.t. 2d projected gaussian parameters
                            ...
                            const S v_G_ddelx = v_Gg_ddelx * (1 - s_weight) + v_Gs_ddelx * s_weight;
                            const S v_G_ddely = v_Gg_ddely * (1 - s_weight) + v_Gs_ddely * s_weight;
                ...
                warpSum<decltype(warp), S>(v_steepness_local, warp);
                ...
                if (warp.thread_rank() == 0)
                {
                    ...
                    gpuAtomicAdd(v_steepnesses + g, v_steepness_local);
                }
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_bwd_2dgss_kernel_with_dim(
        // Gaussian parameters
        ...
        bool absgrad)
    {
        ...
        torch::Tensor v_steepnesses = torch::zeros_like(steepnesses);
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_2dgss_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                            \
    case N:                                       \
        return call_bwd_2dgss_kernel_with_dim<N>( \
            means2d,                              \
            steepnesses,                          \
            ray_transforms,                       \
            colors,                               \
