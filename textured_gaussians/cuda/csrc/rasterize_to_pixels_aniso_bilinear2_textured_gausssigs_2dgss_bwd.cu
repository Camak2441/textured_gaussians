...
#include "types.cuh"
#include "utils.cuh"
#include "filters/anisotropic_bilinear2.cuh"
...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_aniso_bilinear2_textured_gausssigs_kernel(
        ...
        S *__restrict__ v_densify)
    {
        ...
        // number of batches needed to process all sigmoids in this tile
        ...
        // the contribution from sigmoids behind the current one
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            // so we are processing the sigmoids in the order of closest to furthest
            ...
            block.sync();
            // loops through the sigmoids in the current batch for this pixel
            ...
            for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
            {
                ...
                if (valid)
                {
                    ...
                    const vec3<S> s0ray_cross = ray_cross - S(0.5) * dxw - S(0.5) * dyw;
                    ...
                    {
                        s0 = anisotropic_bilinear2::s_to_uv(
                ...
                if (valid)
                {
                    ...
                    // we have d(alpha)/d(c_i) = c_i * G_i * T + [grad contribution from following sigmoids in T term]
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    call_bwd_aniso_bilinear2_gs_kernel_with_dim(
        // Gaussian parameters
        ...
        bool absgrad)
    {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_aniso_bilinear2_textured_gausssigs_kernel<CDIM, float>,
...
#define __GS__CALL_(N)                                         \
    case N:                                                    \
        return call_bwd_aniso_bilinear2_gs_kernel_with_dim<N>( \
            means2d,                                           \
            steepnesses,                                       \
            ray_transforms,                                    \
            colors,                                            \
